import { Component, inject, signal } from '@angular/core';
import { FormGroup } from '@angular/forms';
import { HelpersService } from '@core/services/helpers.service';
import { MESSAGES } from '@core/constants/messages';
import { LABEL_BUTTONS, LABELS } from '@core/constants/labels';
import { TypeSubmitEnum } from '@core/enums/type-submit';
import { GenericTableComponent } from '@shared/components/generic-table/generic-table.component';
import { FormUtils } from '@core/utils/form-groups';
import { SEVERITY_ENUM } from '@core/enums/severity.enum';
import { TypeMaterialService } from '../../services/type-material.service';
import { TypeMaterial } from '@core/interfaces/type-material.interface';

@Component({
    selector: 'app-modal-form',
    templateUrl: './modal-form.component.html',
    standalone: false
})
export class ModalFormComponent {

  private helpersService = inject(HelpersService);
  private typeMaterialService = inject(TypeMaterialService);

  public readonly labels = LABELS;
  public readonly messages = MESSAGES;
  public readonly buttons = LABEL_BUTTONS;
  public selectedTypeMaterial: TypeMaterial;
  public openModal: boolean = false;
  public tittleForm: string = "";
  public formTypeMaterial: FormGroup = FormUtils.getDefaultTypeMaterialFormGroup();
  private tableComponent: GenericTableComponent<TypeMaterial>;
  private isEdit = signal<boolean>(false);


  ngOnInit() {
    this.registerTableComponentListener();
    this.waitForDataSelection();
    this.typeMaterialService.eventFormComponent.emit(this);
  };
  
  public hideModal() {
    this.openModal = false;
  };
 
  private openCreate() {
    this.reset();
    this.tittleForm = "Crear tipo de material";
    this.isEdit.set(false);
    this.openModal = true;
  };

  private openEdit(id: number) {
    this.reset();
    this.tittleForm="Editar tipo de material";
    this.typeMaterialService.findById(id).subscribe({
      next: (res) => {
        this.isEdit.set(true);
        this.openModal=true;
        this.updateFormValues(res);
      }
    })
  };

  private updateFormValues(TypeMaterial: TypeMaterial) {
    this.formTypeMaterial.patchValue({
      ...TypeMaterial
    });
  }

  public saveTypeMaterial() {
    if (this.formTypeMaterial.valid ) {
      this.isEdit()? this.submit(TypeSubmitEnum.UPDATE): this.submit(TypeSubmitEnum.CREATE);
    }
  };

  private reset(): void {
    this.formTypeMaterial.reset();
  };

  private submit(type: TypeSubmitEnum) {
    const data: TypeMaterial = {
      ...this.formTypeMaterial.value,
    };
    const service = type == TypeSubmitEnum.CREATE
      ? this.typeMaterialService.create(data)
      : this.typeMaterialService.update(this.selectedTypeMaterial.id, data)
    service.subscribe({
      next: () => {
        const message = type == TypeSubmitEnum.CREATE
          ? MESSAGES.recordCreated
          : MESSAGES.recordUpdated;
        this.helpersService.messageNotification(SEVERITY_ENUM.success, "Correcto", message);
        this.hideModal();
        this.reset();
      }, complete: () => {
        this.tableComponent.reload();
      }
    })
  }

  private registerTableComponentListener() {
    this.typeMaterialService.eventTableComponent.subscribe((tableComponent) => {
      this.tableComponent = tableComponent;
    });
  }

  private waitForDataSelection() {
    this.typeMaterialService.getSelectedRow().subscribe((TypeMaterial) => this.selectedTypeMaterial = TypeMaterial);
  }
  
}
